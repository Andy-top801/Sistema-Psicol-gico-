...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with schema_context(get_public_schema_name()):
            ...
            if not Usuario.objects.filter(email='beto.caleb.delgado.rojas@gmail.com').exists():
                superadmin = Usuario.objects.create_superuser(
                    email='beto.caleb.delgado.rojas@gmail.com',
                    password='Admin1234*',
                    nombre='Alberto',
                    apellido='Delgado'
                )
                self.stdout.write(self.style.SUCCESS("[OK] SuperAdmin creado (beto.caleb.delgado.rojas@gmail.com)"))
