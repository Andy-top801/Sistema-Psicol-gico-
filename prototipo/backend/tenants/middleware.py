...
class SigepsiTenantMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        # 1.1 Si no vino header explícito, intentar resolver tenant desde el token JWT (Bearer)
        if not tenant_header:
            ...
            if auth_header and auth_header.startswith('Bearer '):
                ...
                try:
                    ...
                    t_identifier = payload.get('tenant_slug') or payload.get('tenant_id') or payload.get('schema_name')
                    if t_identifier and t_identifier.lower() != 'public':
                        tenant_header = t_identifier
                    elif not t_identifier and payload.get('user_id'):
                        u_id = payload.get('user_id')
                        from accounts.models import Usuario
                        for t_cand in Tenant.objects.exclude(schema_name='public').filter(activo=True):
                            with schema_context(t_cand.schema_name):
                                if Usuario.objects.filter(id=u_id).exists():
                                    request.tenant = t_cand
                                    connection.set_tenant(t_cand)
                                    return None
                except Exception:
                    pass
